...
#导入相关库
#pdf转字符串
from .pdf_related import getstrfrompdf



# 进口带板箱信息的wa解析
def extract_wa(path,filename):
    #调用pdf_realted库从pdf中提取str
    str=getstrfrompdf(path,filename)
    ...
    #获得航班信息
    dateinfo=re.search('[0-9,A-Z]{2}[ ]{0,2}[0-9]{1,4}/[0-9]{1,2}-[A-Z]{3}-[0-9]{4}',original_list[3])[0].split('/')
    #航班号
    ...
    def getdetail(str):
        ...
        for item in str_list:
            ...
            special_dic[awb]={
                ...
                'arr':arrs,
                #本次航班到达总件数
                'arrtotalpiece':arr_total_p,
                #本次航班到达总重
                'arrtotalweight':arr_total_weight,                
            }
        return special_dic
    ...
    # 2.航班日期  str
    # 3.到达重量  float
    # 4.rev_local的运单信息   dict
    # 5.rev_trst的运单信息    dict
    # 6.nrev_local的运单信息  dict
    # 7.nrev_trst的运单信息   dict
